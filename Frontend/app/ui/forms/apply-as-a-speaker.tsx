...
export default function ApplySpeakerForm(): JSX.Element {
  const [isSubmitting, setIsSubmitting] = useState<boolean>(false);
  const [isSubmitted, setIsSubmitted] = useState<boolean>(false);
  const confettiRef = useRef<HTMLCanvasElement>(null);

  const handleSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
    const API_URI = process.env.NEXT_PUBLIC_BACKEND_URI + "/api/speaker/apply";
    e.preventDefault();
      ...
      >
        <div className="backdrop-blur-lg bg-white/70 p-8 rounded-xl shadow-2xl border border-yellow-500">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="block text-black font-medium">About you</label>
              <textarea
                disabled={isSubmitting || isSubmitted}
                minLength={10}
                name="about"
